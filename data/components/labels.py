...
class Textbox(pg.sprite.Sprite, tools._KwargMixin):
    ...
    def __init__(self, rect_attr, *groups, **kwargs):
        ...
        if self.cursor_image is None:
            if self.cursor_size is None:
                self.cursor_size = max(1, self.font_size // 8), self.font_size
    ...
    def make_image(self):
        ...
        if self.active and self.cursor_active:
            ...
            midright = (left, self.buffer_label.rect.centery)
            rect = self.cursor_image.get_rect(midright=midright) #midleft=midleft)
